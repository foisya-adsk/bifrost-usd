...
Mesh::Mesh(const Bifrost::Object& object, const size_t index) {
    ...
    m_children_map[pxr::SdfPath{name}] = CreateHdSceneIndexMesh(object);
}

const pxr::TfToken& Mesh::getSceneIndexPrimTypeName() const {
    return HdPrimTypeTokens->mesh;
}

const pxr::HdDataSourceLocator& Mesh::TopologyLocator() const {
