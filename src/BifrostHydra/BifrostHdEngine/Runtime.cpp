...
Runtime& Runtime::Get() {
    if (!g_runtime) {
        ...
        auto config = Bifrost::createObject();
        if (BifrostGraph::Executor::Utility::getConfigFromEnvironment(
                *config)) {
            auto configFiles = Amino::any_cast<Amino::Ptr<Amino::StringList>>(
                (*config).getProperty("bifrost_pack_config_files"));
            if (configFiles) {
